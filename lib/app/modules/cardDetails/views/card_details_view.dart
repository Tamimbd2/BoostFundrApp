...
class CardDetailsView extends GetView<CardDetailsController> {
          ...
          return const Center(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 16),
                Text('Loading deal details...', style: TextStyle(color: Colors.white54, fontSize: 12)),
              ],
            ),
          );
        }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildLockedSection(
                      ...
                      child: _buildInfoSection('Business Model', Icons.business_center_outlined, controller.businessModel),
                    ),
                    const SizedBox(height: 14),
                    ...
                    _buildLockedSection(
                      ...
                      child: Column(
                        children: [
                          _buildInfoSection('Market', Icons.pie_chart_outline, controller.market),
                          const SizedBox(height: 14),
                          _buildInfoSection('Geography', Icons.public, controller.geography),
                        ],
                      ),
                    ),
                    const SizedBox(height: 14),
                    ...
                    _buildLockedSection(
                      ...
                      child: Column(
                        children: [
                          _buildInfoSection('Founder Details', Icons.person_outline, controller.founderDetails),
                          const SizedBox(height: 14),
                          _buildInfoSection('Team Info', Icons.groups_outlined, controller.team),
                        ],
                      ),
                    ),
                    const SizedBox(height: 14),
                    ...
                    _buildLockedSection(
                      fieldName: 'traction',
                      child: _buildInfoSection('Traction Details', Icons.trending_up, controller.traction),
                    ),
                    const SizedBox(height: 14),

                    _buildLockedSection(
                      fieldName: 'useOfFunds',
                      child: _buildInfoSection('Use of Funds', Icons.account_balance_wallet_outlined, controller.useOfFunds),
                    ),
                    const SizedBox(height: 24),
  ...
  Widget _imagePlaceholder(Color neonGreen) => Container(
    ...
    child: Icon(Icons.rocket_launch_outlined, color: neonGreen.withOpacity(0.2), size: 48),
  );

  Widget _buildStatusRow(Color neonGreen) {
    final status = controller.status;
    final statusColor = status == 'approved' ? neonGreen
        : status == 'draft' ? const Color(0xFFFBBF24) : Colors.redAccent;
    ...
    return Column(
      ...
      children: [
        ...
        if (controller.tagline.isNotEmpty) ...[
          ...
          Text(controller.shortPitch, style: TextStyle(color: Colors.white.withOpacity(0.5), fontSize: 13)),
        ],
        if (controller.location.isNotEmpty && controller.location != '—') ...[
          const SizedBox(height: 10),
          Row(children: [
            Icon(Icons.location_on_outlined, color: Colors.white.withOpacity(0.3), size: 14),
            const SizedBox(width: 4),
            Text(controller.location, style: TextStyle(color: Colors.white.withOpacity(0.3), fontSize: 12)),
          ]),
        ],
      ],
    );
          ...
          Row(
            ...
            children: [
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('$cur ${fmt(raised)}', style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)),
                Text('of $cur ${fmt(goal)}', style: TextStyle(color: Colors.white30, fontSize: 12)),
              ]),
              Text('${(ratio * 100).toInt()}%', style: TextStyle(color: neonGreen, fontSize: 18, fontWeight: FontWeight.bold)),
            ],
          ),
          const SizedBox(height: 12),
  ...
  Widget _buildProfileScoreCard(Color neonGreen) {
    ...
    final color = score >= 80 ? neonGreen : score >= 50 ? Colors.orange : Colors.red;
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const Text(' / 100', style: TextStyle(color: Colors.white30, fontSize: 14)),
              const Spacer(),
              Icon(Icons.shield_outlined, color: color.withOpacity(0.5), size: 32),
            ],
          ),
        ],
      ),
    ...
    return _glassCard(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          Text(content, style: TextStyle(color: Colors.white.withOpacity(0.7), fontSize: 13, height: 1.6)),
        ],
      ),
    );
  }
  ...
  Widget _buildTractionChips(Color neonGreen) {
    return Wrap(
      ...
      children: controller.tractionHighlights.map((t) => Container(
        ...
        child: Text(t, style: TextStyle(color: neonGreen.withOpacity(0.8), fontSize: 11)),
      )).toList(),
    );
  }
  ...
  Widget _buildDeadlineRow(Color neonGreen) {
    ...
    return _glassCard(
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          Text('Campaign Ends', style: TextStyle(color: Colors.white.withOpacity(0.5), fontSize: 14)),
          Text('$days Days Left', style: TextStyle(color: color, fontWeight: FontWeight.bold)),
        ],
      ),
    );
  }
  ...
  Widget _buildSectionTitle(String title) => Text(title, style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold));

  Widget _buildLockedSection({required String fieldName, required Widget child}) {
    return Obx(() {
        ...
        child: Stack(
          ...
          children: [
            ...
            Padding(
              ...
              child: Column(
                children: [
                  const Icon(Icons.lock_person_outlined, color: Color(0xFF00FF88), size: 32),
                  const SizedBox(height: 8),
                  Text('Unlock with $target', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 12),
                  ElevatedButton(
  ...
  Widget _buildPrivateDocumentsSection() => _glassCard(child: const Center(child: Text('Confidential documents locked', style: TextStyle(color: Colors.white30, fontSize: 12))));
}
